...
import 'package:fulus_mobile/domain/repositories/stock_movement_repository.dart';
...
void main() {
  ...
  SyncQueueItem itemFor(String localId, {String operation = 'create'}) => SyncQueueItem(
        id: 'q1', entityType: 'stock_movement', entityLocalId: localId,
  ...
  test('pushes stock-in through Fulus Cloud and reconciles returned stock', () async {
    final movement = await stockMovementRepository.recordStockIn(
      const StockInDraft(productLocalId: productLocalId, locationId: locationId, quantity: 20, reason: 'New delivery'),
    );
    ...
    verify(() => fulusSyncApi.submitOperation(
          businessId: 'business-1', operationType: 'stock_movement.create', operationId: 'q1',
          deviceClientId: 'device-client-1', clientReference: movement.localId,
    ...
    final stored = await (db.select(db.stockMovements)
  ...
  test('rejects adjustments instead of inventing a delta', () async {
    final movement = await stockMovementRepository.recordAdjustment(
      const StockAdjustmentDraft(productLocalId: productLocalId, locationId: locationId, newQuantity: 42, reason: 'Recount'),
    );
