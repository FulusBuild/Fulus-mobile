...
class _SellBodyState extends ConsumerState<_SellBody> {
  ...
  Future<void> _add(BuildContext context, ProductWithStock item) async {
    try {
      await context.read<CartCubit>().addProduct(item.product.localId);
  ...
  Future<void> _scan(BuildContext context) async {
    ...
    final match = state.catalog.values.cast<ProductWithStock?>().firstWhere(
      ...
      body: Column(
        children: [
          Padding(
            ...
            child: FulusSearchField(controller: _searchController, hintText: 'Search products…', autofocus: true, onChanged: (v) => setState(() => _query = v)),
          ),
          Padding(
...
class _CatalogList extends StatelessWidget {
  ...
  final _CatalogView view;
  final String query;
  final String? categoryId;
  final bool favoritesOnly;
  final Set<String> favoriteIds;
  final bool favoritesLoaded;
  final Future<void> Function(ProductWithStock) onAdd;
  final Future<void> Function(String) onFavorite;
  final VoidCallback onClear;

  @override
  Widget build(BuildContext context) {
    if (!view.loaded || !favoritesLoaded) {
      return ListView.separated(padding: const EdgeInsets.fromLTRB(AppSpacing.lg, AppSpacing.sm, AppSpacing.lg, AppSpacing.xxxl), itemCount: 6, separatorBuilder: (_, __) => const SizedBox(height: AppSpacing.sm), itemBuilder: (_, __) => const FulusDelayedSkeleton(skeleton: FulusCardSkeleton()));
    }
    ...
    final products = view.catalog.values.where((item) {
      ...
      final fav = (favoriteIds.contains(b.product.localId) ? 1 : 0) - (favoriteIds.contains(a.product.localId) ? 1 : 0);
      return fav != 0 ? fav : a.product.name.compareTo(b.product.name);
    });
    ...
    if (products.isEmpty) {
      return FulusEmptyState(
        ...
        body: favoritesOnly ? 'Star your fastest-selling products to keep them one tap away.' : q.isEmpty ? 'No products in this category yet.' : 'Nothing matches "$query" — try a different search, or use Quick Sale.',
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, i) => FulusProductRow(
        productWithStock: products[i],
        currencySymbol: view.currency,
        isFavorite: favoriteIds.contains(products[i].product.localId),
        onAdd: () => onAdd(products[i]),
        onToggleFavorite: () => onFavorite(products[i].product.localId),
      ),
    );
  }
}
