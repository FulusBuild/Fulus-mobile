...
class FulusSyncApi {
  ...
  Future<FulusSyncPullResponse> pullChanges({required String businessId, int cursor = 0, int limit = 100}) async {
    try {
      final response = await _client.dio.get(_functionBaseUrl, queryParameters: {'business_id': businessId, 'cursor': cursor, 'limit': limit}, options: Options(headers: _headers()));
      return FulusSyncPullResponse.fromJson(Map<String, dynamic>.from(response.data as Map));
  ...
  Future<Map<String, dynamic>> submitOperation({
    ...
    try {
      final rawPayload = payload is Map ? Map<String, dynamic>.from(payload) : <String, dynamic>{};
      ...
      if (operationType == 'sale.create' || operationType == 'customer.create' || operationType == 'customer.repayment') {
        body
          ...
          ..['action'] = switch (operationType) {
            ...
            _ => 'customer_repayment',
          };
      }
      if (operationType.startsWith('product.') || operationType.startsWith('category.') || operationType.startsWith('supplier.')) {
      ...
      final response = await _client.dio.post(_functionBaseUrl, data: body, options: Options(headers: _headers(deviceClientId: deviceClientId)));
  ...
  Map<String, dynamic> _normalizeOperationResponse(Map<String, dynamic> result, {required String operationType}) {
    ...
    } else if ((operationType.startsWith('product.') || operationType.startsWith('category.') || operationType.startsWith('supplier.')) && data['item'] is Map) {
  ...
  Map<String, String> _headers({String? deviceClientId}) => {
    ...
    if (_client.serverAccessToken != null) 'Authorization': 'Bearer ${_client.serverAccessToken}',
  };
}

class FulusSyncPullResponse {
  const FulusSyncPullResponse({required this.changes, required this.cursor, required this.nextCursor, required this.hasMore});
  ...
  factory FulusSyncPullResponse.fromJson(Map<String, dynamic> json) {
    ...
    return FulusSyncPullResponse(
      changes: rawChanges.map((item) => FulusSyncChange.fromJson(Map<String, dynamic>.from(item as Map))).toList(growable: false),
...
class FulusSyncChange {
  const FulusSyncChange({required this.sequence, required this.entityType, required this.entityId, required this.operation, required this.payload, required this.createdAt});
  final int sequence;
