-- Fix the live catalog mutation conflict query.
-- The PL/pgSQL variable entity_id must not collide with sync_changes.entity_id.
...
begin
  ...
  if target_operation='delete' then
    ...
    -- Product creation is allowed to carry an initial local stock count.
    -- That count must be materialized in the authoritative stock ledger in
    -- the same transaction as the product row; otherwise the next pull can
    -- legitimately overwrite the local-first stock back to zero.
