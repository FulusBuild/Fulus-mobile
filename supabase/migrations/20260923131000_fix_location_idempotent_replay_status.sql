-- Return a replay-specific status for completed location idempotency operations.
-- The original response remains persisted unchanged; only the replay response is
-- marked already_applied so the HTTP layer can correctly return 200 rather than
-- 201 for a repeated command.
...
begin
  ...
  if idem.completed_at is not null and idem.response_body is not null then
    return jsonb_set(idem.response_body,'{status}','"already_applied"');
  end if;
