...
-- Retries after a committed timeout must return the existing business.
-- The per-user advisory transaction lock also closes the double-submit race.
...
begin
  ...
  if existing_owner_count = 1 then
    ...
    select lm.location_id into new_location_id
  ...
  select r.id into new_admin_role_id
  from public.roles r where r.business_id = new_business_id and r.name = 'admin';

  select r.id into new_manager_role_id
  from public.roles r where r.business_id = new_business_id and r.name = 'manager';

  select r.id into new_cashier_role_id
  from public.roles r where r.business_id = new_business_id and r.name = 'cashier';
