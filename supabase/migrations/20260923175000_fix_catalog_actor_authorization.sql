-- Fix catalog writes invoked through the service-role Edge Function.
-- The verified caller is passed explicitly as target_user_id; auth.uid() is not
-- populated on the service-role client, so has_permission() would reject every
-- legitimate catalog mutation. Also bind the device to the same verified user.
...
begin
  ...
  if not exists(
    ...
    where id=target_device_id and business_id=target_business_id and registered_by=target_user_id and status='active'
  ) then
