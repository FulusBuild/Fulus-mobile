-- Fulus Cloud Sync — idempotent catalog commands
-- Catalog writes are mutable master-data operations. They must use the same
-- operation-id/request-hash contract as financial commands instead of writing
-- tables directly from the Edge Function.
...
begin
  ...
  values(
    ...
    target_operation_id, 'catalog.' || target_entity || '.' || target_operation,
    target_request_hash
  )
    ...
    case target_entity
      when 'categories' then
        if target_id is null then
          ...
          values(target_business_id, trim(target_item->>'name'), nullif(target_item->>'description',''))
          returning id, to_jsonb(categories) into entity_id, row_data;
        else
          update public.categories
      ...
      when 'suppliers' then
        if target_id is null then
          ...
          values(target_business_id, trim(target_item->>'name'), target_item->>'phone', target_item->>'email', target_item->>'address')
          returning id, to_jsonb(suppliers) into entity_id, row_data;
        else
          update public.suppliers
          set name = trim(target_item->>'name'),
      ...
      when 'products' then
        if target_id is null then
          ...
          values(
            target_business_id, trim(target_item->>'name'),
            target_item->>'sku', target_item->>'barcode',
...
revoke execute on function public.cloud_catalog_mutate(uuid,uuid,uuid,text,text,text,uuid,jsonb,text)
