-- Adds an authoritative change-feed boundary to the existing restore snapshot.
-- The boundary is read in the same PostgreSQL statement as the snapshot data,
-- so bootstrap can import the snapshot and then safely resume from this exact
-- sequence without skipping changes created after the snapshot boundary.
