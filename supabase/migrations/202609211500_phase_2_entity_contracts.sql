-- Phase 2: complete the real customer/expense update contracts and establish
-- the missing server-side expense-category entity.
...
create index if not exists expense_categories_business_updated_idx
  on public.expense_categories (business_id, updated_at desc);
...
create policy expense_categories_manage on public.expense_categories
  for all using (
    public.has_permission(expense_categories.business_id, 'finance.manage')
  ) with check (
    public.has_permission(expense_categories.business_id, 'finance.manage')
  );
...
create or replace function public.fulus_api_update_customer(
  ...
  target_is_active boolean, target_request_hash text
)
...
create or replace function public.fulus_api_create_expense_category(
  target_user_id uuid, target_business_id uuid, target_device_id uuid,
  target_operation_id text, target_name text, target_request_hash text
)
returns jsonb language plpgsql security definer set search_path = public
as $$
declare
...
revoke all on function public.fulus_api_update_customer(uuid,uuid,uuid,text,uuid,text,text,text,text,text,numeric,boolean,text) from public,anon,authenticated;
revoke all on function public.fulus_api_update_expense(uuid,uuid,uuid,text,uuid,uuid,numeric,text,text,timestamptz,text) from public,anon,authenticated;
revoke all on function public.fulus_api_create_expense_category(uuid,uuid,uuid,text,text,text) from public,anon,authenticated;

grant execute on function public.fulus_api_update_customer(uuid,uuid,uuid,text,uuid,text,text,text,text,text,numeric,boolean,text) to service_role;
grant execute on function public.fulus_api_update_expense(uuid,uuid,uuid,text,uuid,uuid,numeric,text,text,timestamptz,text) to service_role;
grant execute on function public.fulus_api_create_expense_category(uuid,uuid,uuid,text,text,text) to service_role;
