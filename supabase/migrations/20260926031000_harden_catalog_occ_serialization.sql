-- Qualify the entity id used for the change-feed lookup.
-- The previous migration used the PL/pgSQL variable name entity_id in a query
-- that also exposes sync_changes.entity_id, which PostgreSQL rejects as ambiguous.
...
begin
  ...
  -- Serialize concurrent updates to the same catalog row before evaluating
  -- the optimistic-concurrency cursor. Without the row lock, two transactions
  -- can observe the same pre-update feed state and both commit successfully.
  ...
  if target_operation='delete' then
    ...
    where sc.business_id=target_business_id
      and sc.entity_type=feed_entity
      and sc.entity_id=(row_data->>'id')::uuid;
