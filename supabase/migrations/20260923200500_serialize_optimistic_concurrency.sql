-- Cloud Sync V1 hardening: serialize optimistic-concurrency checks against
-- concurrent mutations of the same server entity. Without a row lock, two
-- requests can both observe no change after the same base cursor and both
-- commit, defeating conflict detection.
--
-- PostgreSQL row locks are held until the surrounding transaction commits,
-- so the second mutation waits for the first mutation and then evaluates
-- its change-feed predicate against the committed state.
