-- Restore snapshot must only reference relations that exist in the production
-- cloud schema. These three local restore sections have no production tables yet:
-- expense_categories, supplier_ledger_entries, tax_remittances. The importer
-- already treats absent optional sections as empty, so omitting them preserves
-- the restore contract without inventing production tables or losing the
-- single-statement snapshot guarantee.
--
-- role_permissions is production-backed but intentionally uses a composite
-- primary key (role_id, permission_id), so it must not order by a nonexistent id.
...
begin
  ...
  -- All successful snapshot reads belong to this one SELECT statement.
  -- Under READ COMMITTED, separate PL/pgSQL SQL statements can use different
  -- snapshots, so authorization and metadata are deliberately in this query.
  with authz as (
  ...
  select jsonb_build_object(
    ...
    'local_restore_notes', jsonb_build_object(
      ...
      'server_schema_optional_sections_omitted', jsonb_build_array('expense_categories','supplier_ledger_entries','tax_remittances'),
